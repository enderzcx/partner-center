...
通过独立审查处理及本地全链路测试后，可设置 `SETTLEMENT_X402_ENABLED=true` 使用已验证镜像。先停止服务并成对备份两个账本，保留旧镜像与配置，再启动新版本。升级后核验旧订单没有被改标为真实x402收款，匿名/推广者不能付款，新订单返回正确402且模拟支付被拒绝。

一旦存在x402付款授权或收款，不得恢复旧账本抹去收款 journal，也不得公开运行忽略x402记录的旧程序。故障时先将此独立站点切为维护响应并保留新账本；核对每份原授权与链上结果后再恢复。无签名的报价验证不等同于真实Fuji收款验收。
