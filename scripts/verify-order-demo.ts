...
const created = await api('/api/demo/orders', { request_id: 'e2e-' + crypto.randomUUID(), payment_amount_minor: '1000' });
const requestId = created.order.requestId;
const replayCreated = await api('/api/demo/orders', { request_id: requestId, payment_amount_minor: '1000' });
assert.equal(replayCreated.order.tradeNo, created.order.tradeNo);
...
const replayAfterPaid = await api('/api/demo/orders', { request_id: requestId, payment_amount_minor: '1000' });
assert.equal(replayAfterPaid.order.tradeNo, created.order.tradeNo);
assert.equal(replayAfterPaid.order.status, 'paid');
